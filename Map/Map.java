...
public class Map implements CarEvents, Time {
    ...
    private void SpawnCar() {
        for (int i = 0; i < carsToAdd; i++) {
            if (cars.size() + 1 <= carLimit) {
                ...
                cars.add(new Car(25f, road.get(0), road.get(0).PickStartingPoint(), road.get(0), road, this));
    ...
    private boolean SpawnCar(SPair<Integer> start, SPair<Integer> end) {
        if (cars.size() + 1 <= carLimit) {
            LinkedList<GridNode> road = new A_star(node_map, start, end).findPathTo();
            cars.add(new Car(25f, road.getFirst(), road.getFirst().PickStartingPoint(), road.getFirst(), road, this));
            return true;
        }
        return false;
    }
